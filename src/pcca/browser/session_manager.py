...
class BrowserSessionManager:
    ...
    async def export_netscape_cookies(self, *, platform: str, filename: str = "cookies.txt") -> Path | None:
        """Export current Playwright cookies for tools such as yt-dlp.

        yt-dlp consumes the old Netscape cookies.txt format. We keep the file
        under PCCA's platform profile rather than reading Chrome's locked
        SQLite cookie DB directly, which avoids platform-specific decryption
        and profile-lock issues.
        """
        context = await self.get_context(platform)
        cookies = await context.cookies()
        if not cookies:
            return None
        path = self.profiles_root / platform / filename
        path.parent.mkdir(parents=True, exist_ok=True)
        lines = [
            "# Netscape HTTP Cookie File",
            "# Generated by PCCA from the local browser session.",
        ]
        for cookie in cookies:
            ...
            try:
                expires_int = int(float(expires)) if expires is not None and float(expires) > 0 else 0
        ...
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        logger.info("Exported %d cookie(s) to Netscape file platform=%s path=%s", len(cookies), platform, path)
        return path
